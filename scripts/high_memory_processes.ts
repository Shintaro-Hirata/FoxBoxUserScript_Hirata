// ============================================================================
// Resource monitor: high memory process filter + ECU totals
//
...
type CpuSample = {
  ...
  active_ns: bigint; // cpu_active_time の累積値
...
function computeCpuUsagePercent(
  ...
  cpuSamples.push({ t_ns: sampleTimeNs, active_ns: activeNs, total_ns: totalNs });

  // 時間窓と件数上限で古いサンプルを間引く
  const cutoff = sampleTimeNs - BigInt(Math.max(1, Math.round(windowMs))) * BigInt(1000000);
  while (cpuSamples.length > 0 && cpuSamples[0]!.t_ns < cutoff) {
    cpuSamples.shift();
  }
  while (cpuSamples.length > MAX_CPU_SAMPLES) {
    cpuSamples.shift();
  }
  ...
  const oldest = cpuSamples[0]!;
  const newest = cpuSamples[cpuSamples.length - 1]!;
...
export default function script(
  ...
  if (cpuInfo !== undefined) {
    ...
    const totalNs = toBig(cpuInfo.cpu_total_time?.duration);
    if (totalNs > BigInt(0)) {
      ...
      cpuUsagePercent = computeCpuUsagePercent(sampleTimeNs, activeNs, totalNs, cpuWindowMs);
      cpuValid = true;
    }
  }
